...
class ProtectedSong(object):
    ...
    def save_secured_song_to_wave(self, file_location):
        ...
        tablua_recta = [bytes(range(0,256))[i:]+bytes(range(0,256))[:i] for i in range(0,256)]
        secret='saippuakivikauppais'
        i = 0
        for val in self.full_song:
            i+=1
            #print(val)
            val=tablua_recta[val][ord(secret[i%19])]
            #print(val)
            protected_wav_val = struct.pack('<B', val) #char... ;)
    ...
    def read_song(self, path, metadata_frames=0):
        ...
        vals = np.frombuffer(song.readframes(song.getnframes()), dtype=np.uint8) #changed for vigenere
        song.close()
        return vals, song
